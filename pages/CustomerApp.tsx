...
import { MapPin, Phone, Info, ChevronRight, ArrowLeft, Home, Clock, Utensils, Sparkles } from 'lucide-react';

// --- Helper Functions ---
const optimizeImageUrl = (url: string, width: number): string => {
    if (!url || url.includes('via.placeholder.com')) {
        return url; // Don't process placeholder images
    }
    // Using a free, open-source image proxy service to resize and convert images on the fly.
    // This is a great way to optimize without needing your own backend for image processing.
    const serviceUrl = `https://images.weserv.nl/?url=`;
    return `${serviceUrl}${encodeURIComponent(url)}&w=${width}&q=80&output=webp`;
};
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden" style={bgStyle}>
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center px-4 sm:px-6 pt-12 sm:pt-16 pb-6 max-w-6xl mx-auto w-full">
        ...
        <div className="text-center mb-10 sm:mb-12 animate-slideIn">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-6xl font-black mb-3 drop-shadow-sm tracking-tight mix-blend-overlay" style={{ color: branding.backgroundColor }}>{branding.restaurantName}</h1>
            ...
            return (
              <button key={branch.id} onClick={() => onSelect(branch)} className="group relative p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] shadow-xl shadow-black/5 overflow-hidden text-left transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl" style={cardStyle}>
                ...
                <div className="flex items-center gap-4 pl-3 sm:pl-4">
                   {branch.logoUrl ? <img src={branch.logoUrl} alt={branch.name} className="w-16 h-16 sm:w-24 sm:h-24 rounded-xl sm:rounded-2xl object-cover shadow-lg border group-hover:scale-105 transition-transform" style={{ borderColor: branding.backgroundColor }} /> : <div className="w-16 h-16 sm:w-24 sm:h-24 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-inner" style={{ backgroundColor: branding.backgroundColor, color: branding.mutedColor }}><MapPin size={36} /></div>}
    ...
    <div className="flex flex-col h-screen font-sans overflow-hidden" style={bgStyle}>
      ...
      <div className="flex-1 overflow-y-auto relative" ref={scrollContainerRef}>
         <div className="relative w-full h-48 md:h-72 overflow-hidden shrink-0" style={{ backgroundColor: branding.textColor }}>
             <img src={branding.headerImageUrl} className="w-full h-full object-cover opacity-60" alt="Cover" onError={(e) => (e.target as HTMLImageElement).src = 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?q=80&w=2000&auto=format&fit=crop'}/>
            ...
            {activeDishes.length === 0 ? <div className="flex flex-col items-center justify-center h-64 animate-slideIn" style={mutedStyle}><div className="w-24 h-24 rounded-full shadow-sm flex items-center justify-center mb-4" style={cardStyle}><Info className="w-10 h-10 opacity-50" /></div><h3 className="text-xl font-bold" style={textStyle}>Hozircha bo'sh</h3><p>Ushbu kategoriyada taomlar mavjud emas.</p></div> : (
               <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-4 sm:gap-6">
                  {activeDishes.map((dish, index) => (
                     <div key={dish.id} className={`group relative rounded-[2rem] p-3 sm:p-4 shadow-lg shadow-black/5 border border-transparent hover:shadow-xl flex flex-col h-full animate-slideIn transition-transform duration-300 hover:-translate-y-1 ${dish.isFeatured ? 'col-span-2' : ''}`} style={{ backgroundColor: branding.cardColor, animationDelay: `${index * 0.05}s` }}>
                        <div className={`relative w-full ${dish.isFeatured ? 'aspect-video' : 'aspect-[5/4]'} rounded-2xl sm:rounded-[1.75rem] overflow-hidden mb-4 shadow-inner`} style={{ backgroundColor: branding.backgroundColor }}>
                           <img src={optimizeImageUrl(dish.imageUrls[0], dish.isFeatured ? 800 : 400)} alt={dish.name} loading="lazy" className="w-full h-full object-cover snap-center shrink-0 transform transition-transform duration-700 group-hover:scale-105" onError={(e) => { (e.target as HTMLImageElement).src = 'https://via.placeholder.com/400?text=No+Image' }}/>
                        ...
                        <div className="flex-1 flex flex-col px-1 sm:px-2 pb-1">
                           <h3 className="font-extrabold leading-tight line-clamp-2 text-base sm:text-xl" title={dish.name} style={textStyle}>{dish.name}</h3>
