...
export const StoreProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateBranding = async (settings: Partial<Branding>) => {
    const updatedBranding = await apiRequest(`${API_BASE_URL}/branding`, {
      method: 'PUT',
  ...
  const addBranch = async (branchData: Omit<Branch, 'id'>) => {
    const newBranch = await apiRequest(`${API_BASE_URL}/branches`, {
      method: 'POST',
  ...
  const updateBranch = async (id: string, data: Partial<Branch>) => {
    const updatedBranch = await apiRequest(`${API_BASE_URL}/branches/${id}`, {
      method: 'PATCH',
  ...
  const addCategory = async (name: string, viewType: CategoryViewType = 'grid') => {
    const newCatData = {
      name,
      viewType,
      sortOrder: (categories.length > 0 ? Math.max(...categories.map(c => c.sortOrder)) : 0) + 1
    };
    const newCat = await apiRequest(`${API_BASE_URL}/categories`, {
      method: 'POST',
  ...
  const updateCategory = async (id: string, data: Partial<Category>) => {
    const updatedCategory = await apiRequest(`${API_BASE_URL}/categories/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    });
  ...
  const addDish = async (dishData: Omit<Dish, 'id'>) => {
    ...
    const newDish = await apiRequest(`${API_BASE_URL}/dishes`, {
      method: 'POST',
  ...
  const updateDish = async (id: string, data: Partial<Dish>) => {
    const updatedDish = await apiRequest(`${API_BASE_URL}/dishes/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    });
  ...
  const reorderDishes = async (updatedDishes: Dish[]) => {
    const updatePromises = updatedDishes.map(dish =>
      apiRequest(`${API_BASE_URL}/dishes/${dish.id}`, {
        method: 'PATCH',
        body: JSON.stringify({ sortOrder: dish.sortOrder }),
      })
    );
  ...
  const moveDish = async (id: string, direction: 'up' | 'down') => {
    ...
    await Promise.all([
      updateDish(targetDish.id, { sortOrder: adjacentDish.sortOrder }),
      updateDish(adjacentDish.id, { sortOrder: targetDish.sortOrder })
    ]);
  };
  ...
  return (
    <StoreContext.Provider value={value as StoreContextType}>
      {children}
